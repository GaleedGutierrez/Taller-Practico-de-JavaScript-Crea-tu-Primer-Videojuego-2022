...
const moveCharacterWithKeys = (event) => {
    const KEY_PRESS = event.code;
    if (!KEYS[KEY_PRESS])
        return;
    console.log(KEYS[KEY_PRESS]);
};
const moveCharacterWithButtons = (event) => {
...
let elementSize = 0;
let fontSize = 0;
const BUTTON_LEFT = document.querySelector('#main__button-left-id');
const BUTTON_UP = document.querySelector('#main__button-up-id');
const BUTTON_RIGHT = document.querySelector('#main__button-right-id');
const BUTTON_DOWN = document.querySelector('#main__button-down-id');
const KEYS = {
    ArrowLeft: 'LEFT',
    ArrowUp: 'UP',
    ArrowRight: 'RIGHT',
